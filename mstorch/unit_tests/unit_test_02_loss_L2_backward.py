...
mlcv_l2.forward(input, target)
torch_forward = torch.sqrt(torch_l2(input_t, target_t))

# Calculate the gradient
mlcv_backward = mlcv_l2.backward()  # Depends on mlcv_l2.forward()

torch_forward.sum().backward()
torch_backward = input_t.grad.detach().numpy()
